...
export class CatalogService {
	...
	async doCauseSearch(
		...
		const { textToMatch = '', sortBy = 'CauseLupID', sort = 'asc' } = filters;
		let counter = [{ count: 0 }];
		let list = [];

		if (textToMatch) {
			counter = await db('Burial.CauseLookup')
				.where('Burial.CauseLookup.Cause', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.CauseLookup').count('CauseLupID', {
				as: 'count',
			});
	...
	async doCemetarySearch(
		...
		if (textToMatch) {
			counter = await db('Burial.CemetaryLookup')
				.where('Burial.CemetaryLookup.Cemetary', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.CemetaryLookup').count('CemetaryLupID', {
				as: 'count',
			});
	...
	async doMembershipSearch(
		...
		if (textToMatch) {
			counter = await db('Burial.MembershipLookup')
				.where('Burial.MembershipLookup.Membership', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.MembershipLookup').count('MembershipLUpID', {
				as: 'count',
			});
	...
	async doOccupationSearch(
		...
		if (textToMatch) {
			counter = await db('Burial.OccupationLookup')
				.where('Burial.OccupationLookup.Occupation', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.OccupationLookup').count('OccupationLupID', {
				as: 'count',
			});
	...
	async doRelationshipSearch(
		...
		if (textToMatch) {
			counter = await db('Burial.RelationLookup')
				.where('Burial.RelationLookup.Relationship', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.RelationLookup').count('RelationLUpID', {
				as: 'count',
			});
	...
	async doReligionSearch(
		...
		if (textToMatch) {
			counter = await db('Burial.ReligionLookup')
				.where('Burial.ReligionLookup.Religion', 'like', `%${textToMatch}%`)
			...
			counter = await db('Burial.ReligionLookup').count('ReligionLUpID', {
				as: 'count',
			});
	...
	async modifyAssetType(id: number, item: any) {
		return await db('InterpretiveSite.AssetTypeLookup')
			.update(item)
			.where('InterpretiveSite.AssetTypeLookup.TypeLUpID', id);
	}
	...
	async doAssetTypeSearch(
		page: number,
		limit: number,
		offset: number,
		filters: any
	) {
		const { textToMatch = '', sortBy = 'TypeLUpID', sort = 'asc' } = filters;
		let counter = [{ count: 0 }];
		let list = [];
		...
		if (textToMatch) {
			...
			counter = await db('InterpretiveSite.AssetTypeLookup').count(
				'TypeLUpID',
				{ as: 'count' }
			);
	...
	async modifyCategory(id: number, item: any) {
		return await db('InterpretiveSite.AssetCategoryLookup')
			.update(item)
			.where('InterpretiveSite.AssetCategoryLookup.CatLUpID', id);
	}
	...
	async doCategorySearch(
		page: number,
		limit: number,
		offset: number,
		filters: any
	) {
		const { textToMatch = '', sortBy = 'CatLUpID', sort = 'asc' } = filters;
		let counter = [{ count: 0 }];
		let list = [];
		...
		if (textToMatch) {
			...
			counter = await db('InterpretiveSite.AssetCategoryLookup').count(
				'CatLUpID',
				{ as: 'count' }
			);
	...
	async modifyRoute(id: number, item: any) {
		return await db('InterpretiveSite.RouteLookup')
			.update(item)
			.where('InterpretiveSite.RouteLookup.RouteLUpID', id);
	}
	...
	async doRouteSearch(
		...
		const { textToMatch = '', sortBy = 'RouteLUpID', sort = 'asc' } = filters;
		let counter = [{ count: 0 }];
		let list = [];
		...
		if (textToMatch) {
			...
			counter = await db('InterpretiveSite.RouteLookup').count('RouteLUpID', {
				as: 'count',
			});
	...
	async modifyMaintainer(id: number, item: any) {
		return await db('InterpretiveSite.MaintOwnLookup')
			.update(item)
			.where('InterpretiveSite.MaintOwnLookup.MaintOwnLUpID', id);
	}
	...
	async doMaintainerSearch(
		...
		if (textToMatch) {
			...
			counter = await db('InterpretiveSite.MaintOwnLookup').count(
				'MaintOwnLUpID',
				{ as: 'count' }
			);
